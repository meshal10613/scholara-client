...
const Register = () => {
    const {registerUser, updateUserProfile, setUser, loginWithGoogle} = useAuthContext();
    ...
    return (
        <div className="flex max-h-screen items-center justify-center transition-colors">
            <div className='hidden xl:block flex-1 bg-primary'>
                <Lottie animationData={registerImg} loop={true} className='w-fit'/>
            </div>
            <div className='min-w-md flex flex-col items-center justify-center gap-10 flex-1'>
                <Logo/>
                <div className="w-full max-w-md p-8 rounded-2xl shadow-xl">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                        ...
                        {
                            profilePic ? <>
                            <img src={profilePic} alt="" className='w-12 h-12 rounded-full mx-auto border'/>
                            </> :
                            <div className='flex flex-col'>
                                <input type="file" 
                                placeholder="Up" onChange={handleImagePreview} className='input w-fit border-2 border-gray-300 cursor-pointer'/>
                            </div>
                        }
                        {/* name */}
                        <div>
                            ...
                            <input
                                ...
                                {...register("password", {
                                    ...
                                    validate: {
                                        ...
                                        hasUppercase: (v) =>
                                            /[A-Z]/.test(v) || "Password must include an uppercase letter",
                                        hasSpecialChar: (v) =>
                                            /[!@#$%^&*(),.?":{}|<>]/.test(v) ||
                    ...
                    <p className="mt-6 text-center text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <Link to="/login" state={from} className="ml-1 text-primary hover:underline">
                            Login
                        </Link>
                    </p>
                    <div className='divider'>Or</div>
                    {/* Google */}
                    <button onClick={handleGoogleRegister} className="btn btn-block bg-white text-black border-[#e5e5e5] mb-2">
